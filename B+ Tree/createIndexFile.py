...
class BPlus:
    ...
    def leafSplit(self,node):
        ...
        self.fileRewrite(node)
        
        
        # Write new node at the end of file

        self.fileRewrite(newNode)
        
        f1.close()
        
        
        #Send the middle key to the parent
        self.parentUpdate(newNode.keys[0],node)    


    def parentUpdate(self,record,left):
        ...
        if parent.keys[blockSize][0] != threshold:
            self.externalSplit(parent)

        # Write this back to the file if parent is not full
        else:
            parent.ptrs.pop()
            parent.keys.pop()
            self.fileRewrite(parent)
        f1.close()

        
    def externalSplit(self,node):
        ...
        # Update parent
        self.parentUpdate(mid_key,node)
        
            
    def fileRewrite(self,node):
        '''
